  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 xl:px-10 2xl:px-14 py-10 sm:py-14">
      ...
      <section className="mb-8">
        ...
        />
        <SolutionDiagram diagram={diagram} />
      </section>
...
function SolutionDiagram({ diagram }: { diagram: BacklogDiagram }) {
  const edgeByPair = new Map(
    diagram.edges.map(edge => [`${edge.from}:${edge.to}`, edge.label])
  )

  return (
    <div className="rounded-2xl border border-black/[0.07] bg-[#fafaf8] p-5 sm:p-6 overflow-hidden">
      <p className="text-[12px] font-semibold text-neutral-800 mb-5">{diagram.title}</p>
          ...
          {diagram.nodes.map((node, index) => {
            ...
            return (
              <div key={node.id} className="flex items-center">
                <div className={`w-44 min-h-28 rounded-2xl border p-4 ${tone}`}>
                  <span className="text-[9px] font-mono uppercase tracking-[0.1em] opacity-50">
                    {node.kind ?? 'process'}
                  </span>
                  <p className="text-[13px] font-semibold leading-snug mt-2">{node.label}</p>
                  {node.detail && (
                    <p className="text-[10px] leading-relaxed mt-1.5 opacity-60">{node.detail}</p>
                  )}
                </div>
                {next && (
                  <div className="w-20 flex flex-col items-center justify-center px-2">
      ...
      {diagram.edges.some(
        edge =>
          !diagram.nodes.some(
            (node, index) =>
              node.id === edge.from && diagram.nodes[index + 1]?.id === edge.to
          )
      ) && (
